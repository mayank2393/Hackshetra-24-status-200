...
const inputCSS = "bg-transparent my-2 border border-white py-1 px-2 rounded-lg focus:outline-none focus:border-[#20BFA9] transition-all w-[80%] mx-auto text-white placeholder-white placeholder-opacity-50 w-full"
...
const SignUp = () => {
    ...
    const submitTechnician = useMutation({
        mutationFn: signupTechnician,
        onSuccess: () => {
            navigator('/login');
        },
    });
    const submitHostelAdmin = useMutation({
        mutationFn: signupHostelAdmin,
        onSuccess: () => {
            navigator('/login');
        },
    });
    const submitCollegeAdmin = useMutation({
    ...
    function handleOnStudentSubmit(e) {
        e.preventDefault();
    ...
    function handleOnTechnicianSubmit(e) {
        e.preventDefault();
        submitTechnician.mutate((technician));
        navigator('/login');
    }
    ...
    function handleHostelAdminChange(e) {
        setHostelAdmin({
            ...hostelAdmin,
            [e.target.name]: e.target.value
        })
    }
    function handleOnHostelAdminSubmit(e) {
        e.preventDefault();
        submitHostelAdmin.mutate((hostelAdmin));
        navigator('/login');
    }
    ...
    function handleCollegeAdminChange(e) {
        setCollegeAdmin({
            ...collegeAdmin,
            [e.target.name]: e.target.value
        })
    }
    function handleOnCollegeAdminSubmit(e) {
        e.preventDefault();
        submitCollegeAdmin.mutate((collegeAdmin));
        navigator('/login');
    }
            ...
            case 'student':
                ...
                content = <>
                    ...
                    <form action="" className="flex flex-col m-2 h-full">
                        <label htmlFor="photo">
                            <img src={profilePicture} className="w-[8rem] h-[8rem] rounded-full mx-auto my-1 bg-slate-500 cursor-pointer border-2 border-black" />
                        </label>
                        <input type="file" name="profile_picture" id="photo" className="hidden" onChange={handleStudentChange} />
