...
const SelectCollege = ({ handleSetCollege }) => {
    const collegeArray = [
        "NIT Kurukshetra",
        "NIT Delhi",
        "NIT Jalandhar",
...
const StudentLogin = ({ college }) => {
    ...
    const handleLogin = async (e) => {
        let domain_id = (domainID).current.value;
        let password = (student_password).current.value;
        let role = "student";
...
const TechnicianLogin = ({ college }) => {
    ...
    const handleTechnicianLogin = async ()=>{
        ...
        const response = await axios.post(`${hostname}/api/login/technician`, data);
        const { technician } = response.data;
        localStorage.clear();
        console.log(response);
        for(const key in technician){
            localStorage.setItem(key, (technician)[key]);
        }
...
const HostelAdminLogin = ({ college }) => {
    ...
    const handleHostelAdminLogin = async ()=>{
        const data = {};
        data.domain_id = (domain_id).current.value;
        data.password = (password).current.value
        data.role = 'hostel_admin';
        const response = await axios.post(`${hostname}/api/login/admin/hostel`, data);
        const { hostelAdmin } = response.data;
        console.log(response);
        localStorage.clear();
        for(const key in hostelAdmin){
            localStorage.setItem(key, (hostelAdmin)[key]);
        }
...
const CollegeAdminLogin = ({ college }) => {
    const [user, setUser] = useState(null);
    const data = {
        college: college,
        role: 'collegeAdmin'
    ...
    const handleCollegeAdminLogin = async ()=>{
        ...
        const response = await axios.post(`${hostname}/api/login/admin/college`, data);
        const { collegeAdmin } = response.data;
        console.log(response);
        localStorage.clear();
        for(const key in collegeAdmin){
            localStorage.setItem(key, (collegeAdmin)[key]);
        }
